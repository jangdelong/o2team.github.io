...
* AppCache：
	...
	AppCache主要利用manifest 文本文件，告知浏览器被缓存的内容以及不缓存的内容。<br>{% pimg wangcainuan/2016-03-16-optimization/离线存储.jpg "离线存储" '{"title":"lixiancunchu","class":"lixiancunchu"}' %}
...
* js的代码优化，如：
	...
	* 减少DOM访问，尽量缓存DOM；
	* 充分利用事件委托；
	* 减少Repaint（重绘）和Reflow（重排）最好通过批量更新元素减少重排次数，如设置类class统一更新样式。
