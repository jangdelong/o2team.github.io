...
本文旨在让大家了解什么是Docker，并带领大家体验Docker的整个使用流程和体验。
...
其实很像虚拟机，但是跟虚拟机比起来 更灵活,速度快，CPU/内存消耗更低，关键是更方便管理

{% pimg lizhi/docker/docker-filesystems-multilayer3.png %}
...
```
详细的阿里云Docker代理加速器文档，可以点击[这里](https://help.aliyun.com/knowledge_detail/5974865.html)
...
{% pimg lizhi/docker/dockerimages.png %}
...
```
退出容器命令行，因为不是后台运行，所以退出后，container也随即关闭了。`-t` 可以让container在后台运行，并可以随时通过`docker attach` 命令进入容器，具体例子可以看[这里](https://docs.docker.com/engine/reference/commandline/attach/),因为不是本文重点就不展开说明了。

`docker ps` 查看当前启动的 container， `-a` 启动和没启动的都展示
```
