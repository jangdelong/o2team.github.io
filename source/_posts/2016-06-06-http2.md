...
现在软硬件都已基本到位，趟雷的已经探好了路，就等后续大军杀到。买房子的时候，都说早买早享受，HTTP/2也一样，房子买晚了，HTTP/2你还要再等么，你要做的其实就是尽早升级尽快享受。
...
请求基本上是6个一组，然后6个完成后再 **串行请求** 下一组。

** HTTP/2 **
{% pimg http://7xt3gj.com1.z0.glb.clouddn.com/Fneyvmj5x-SWWjAxPoNsYO_3Dvst %}

从上面的部分截图，可以清晰看到，所有请求基本都是并行请求，由于数据发送量较大，所以会有阻塞，或等待，这个应该是数据流重新组合等待的过程，大概在100张图片的时候会返回重组一次数据，正是因为这样所以单个请求时间相对更长。但是就整体速度来说 HTTP/2 为1.53s， HTTP1.1为 2.47s。速度快了近**40%**。

{% pimg http://7xt3gj.com1.z0.glb.clouddn.com/FlrXK_Y8m2Vk301nl4dQBuC9Yw_s %}

上面这张图从另一面也可以体现出，并行请求的短板，就是[木桶理论](http://baike.baidu.com/view/47735.htm)。所以请求尽量做到细粒度,能更快返回数据。
...
这里同样我写了一个测试示例 [Demo 雪碧图](https://l-zhi.com:8081/item4-1.html) & [Demo 没有雪碧图](https://l-zhi.com:8081/item4-2.html)，可以通过测试示例，如下图上面为使用雪碧图的页面， 下面是普通的请求页面。
{% pimg http://7xt3gj.com1.z0.glb.clouddn.com/Fp7MpCvakuWNCeo0RoRghoEcpznK %}

由于木桶理论，在非雪碧图请求中，由于最后返回速度决定于那个最慢的请求，所以非雪碧图单张1.3K文件速度，跟雪碧图6.9K速度差不多，甚至还慢一些，虽然如此，我们再看onload事件触发时间，因为多路复用的特性，虽然请求超过4倍但是请求也只多区区119ms，再考虑到雪碧图的维护成本，其实并不推荐再使用雪碧图。
...
HTTP1.1的时候，我们经常会根据当前的页面，将请求合并成一个。HTTP/2中可以更细粒度的组合你的接口，不用再根据页面所需数据，来组合一个专门的无意义的接口了，不怕请求多，就怕请求太大。
...
以前当我们无法快速提升网络速度，无法改变一些硬件上的性能，能做的可能只有代码阶段，然而现在有了一种强劲的性能提升方案，那就是从HTTP/2。
