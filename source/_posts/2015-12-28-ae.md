...
#### 切换到合成手机导出效果&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;AE文件可以导出多种形式文件，主菜单合成渲染（主要合成导出序列帧相关）和导出菜单（文件－>创建－>创建代理）就可以设置需要的格式，常见的格式有：
...
## 三者对比
这里将本例，分别用了3种形式做`h5`动画，[`ae`动画效果](http://www.pfan123.com/ae/video.html)（ae效果可能要注意一下，由于没有找到比较好的视频压缩工具，初始动画有点卡，循环播放是很流畅的）、[`gif`动画效果](http://www.pfan123.com/ae/index.html)、[`canvas`动画效果](http://www.pfan123.com/ae/canvas.html)，看看三者之间的差异，二维码如下：
{% pimg pfan/ae/12.jpg %}	

- `ae动画效果`，动效整体还原度比较好，支持流媒体加载，但是`android`与`ios`展现形式不一样：`android`可以支持自动播放，`ios`不能；`ios`支持`mov`格式，`android`则不支持；`ios`可以隐藏播放器控制条，`android`无法解决；初始效果与文件大小影响比较大。

- `gif动画效果`，帧比较多，在部分低端`android`表现卡顿，`gif`格式图片，无法很好压缩。

-  `canvas`动画效果，动画还原度无法达到`gif`、`ae`的效果，复合动画帧比较多，图片比较大，对做雪碧图影响较大，做出的动画会出现闪烁低端`android`出现明显卡顿。
