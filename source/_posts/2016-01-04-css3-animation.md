...
2. 逐帧动画：
	...
	```
	{% pimg Yettyzyt/css3-animation/1-4.gif [steps中的坑] %}
	（左图：`number`为2；右图：`number`为1）

	第二个参数可选，接受`start`和`end`两个值：指定在每个间隔的**起点**或是**终点**发生阶跃变化。通过[W3C](http://www.w3.org/)中的一张step的工作机制图可以理解：
	{% pimg Yettyzyt/css3-animation/1-2.png [steps中的坑] %}（图片来源：[W3C](http://www.w3.org/TR/css3-transitions/)）
...
{% pimg Yettyzyt/css3-animation/3-1.jpg [简单做动画] %}（案例：iphone6s推广游戏）
...
1. **惯性**
	...
	{% pimg Yettyzyt/css3-animation/4-2.gif [惯性] %}（案例：618 APP返场页H5）
...
2. **透视**
	...
	```
	{% pimg Yettyzyt/css3-animation/4-9.gif [透视] %}（案例：iphone6s推广游戏）
...
4. **跟随**
	...
	**主体与附属物之间动作的重叠和追随**，就是鉴定动作流畅性与自然度好坏的标准。
	...
	{% pimg Yettyzyt/css3-animation/4-18.gif [跟随] %}（案例：拍拍七夕活动页—七叻个夕）
